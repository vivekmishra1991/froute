...
struct route_entry* set_route_entry(struct route_entry *rte,
	...
	//Add this route to the kernel routing table
	add_kernel_rt(rte);
...
int fill_route_table() {
	...
	if (rte_head != NULL) {
		...
		rte_head = set_route_entry(rte_head, localhost, 10, dummy1);
	}
...
void request_routing_table() {
	...
	sendmesg(AF_INET, RIP_PORT, nbr, sockfd, message, 64);
...
//	printf("\nInside resp handler");
	...
	while (num_entries--) {
		if (is_valid_rip_entry(re)) {    //if valid rip_entries
			re->metric = MIN(re->metric + COST, INF);
...
void send_regular_update() {
	...
	while (1) {
		...
		construct_regular_message(message, nbr, rte_head);
		sleep(10);
	}
}
...
void construct_regular_message(char* message, struct in_addr send_to,
	...
	while (traverser != NULL) {
		...
		if (msg_entry_count != MAX_RIP_ENTRY_LIMIT) {
			...
			sendmesg(AF_INET, RIP_PORT, send_to, sockfd, init_msg_ptr, 1024);
